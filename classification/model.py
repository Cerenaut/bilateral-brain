...
class ResNet9(nn.Module):
    def __init__(self, in_channels, act_fn=nn.ReLU):
        super().__init__()
...
class InvertedResNet9(nn.Module):
    def __init__(self, in_channels, k=None, k_percent=None, act_fn=nn.ReLU):
        super().__init__()
        self.conv1 = conv_block(in_channels, 512, act_fn=act_fn)
        self.conv2 = conv_block(512, 256, act_fn=act_fn, pool=True)
        self.res1 = nn.Sequential(conv_block(256, 256, act_fn=act_fn),
                                    conv_block(256, 256, act_fn=act_fn))
        
        self.conv3 = conv_block(256, 128, act_fn=act_fn, pool=True)
        self.conv4 = conv_block(128, 64, act_fn=act_fn, pool=True)
        self.res2 = nn.Sequential(conv_block(64, 64, act_fn=act_fn),
                                    conv_block(64, 64, act_fn=act_fn))
...
class SparseAutoencoder(nn.Module):
    ...
    def __init__(self,
        ...
        if resnet_arch == 'Resnet9':
            self.encoder = ResNet9(in_channels=in_channels, act_fn=act_fn)
        elif resnet_arch == 'InvertedResnet9':
            self.encoder = InvertedResNet9(in_channels=in_channels, act_fn=act_fn)
        self.fc = nn.Sequential(
            nn.AvgPool2d(4),
            nn.Flatten(),
            nn.Dropout(0.3),
            nn.Linear(enc_out_dim, num_classes),
            act_fn(),
        )
...
if __name__ == '__main__':
    ...
    m = SparseAutoencoder(in_channels=3, num_classes=20)
    out = m(inp)
    # print(out.shape)